...
# Original logic preserved, Termux parts replaced with Android native features.

import os, sys, json, time, threading, random, webbrowser, signal, shutil, traceback, socket, urllib.request, gc
from datetime import datetime
from http.server import HTTPServer, BaseHTTPRequestHandler
import atexit

# ===============================
#  ANDROID NATIVE FEATURES (replaces termux)
# ===============================
ANDROID = hasattr(sys, 'getandroidapilevel')
wakeLock = None

def android_notify(title, msg):
    """Safe notification – works on Android and non‑Android"""
    if ANDROID:
        try:
...
if ANDROID:
    ...
    android_notify("Trading Bot", "Bot is active")

# ===============================
#  ORIGINAL IMPORTS (keep yfinance)
# ===============================
try:
...
# ===============================
#  WATCHLIST PERSISTENCE (unchanged)
# ===============================
def save_watchlist():
    global WATCHLIST
    try:
...
def prevent_sleep():
    ...
    def notify_loop():
        while running:
            ...
            if running:
                # calculate holdings safely
                holdings = 0.0
                for sym, lots in positions.items():
                    price = get_price_cached(sym)
                    if price:
                        holdings += sum(l['shares'] for l in lots) * price
                net = cash + holdings
                android_notify("Trading Bot", f"Active | P:{len(positions)} | Net: ${net:.0f}")
...
if __name__ == '__main__':
    ...
    def start_bot():
        ...
        android_notify("Trading Bot", f"Bot started on port {PORT}")

    class TradingApp(App):
        def build(self):
            # start backend in a thread so UI doesn't block
            threading.Thread(target=start_bot, daemon=True).start()
            layout = BoxLayout(orientation='vertical')
            self.status = Button(text='Starting bot...', size_hint=(1, 0.1))
            ...
            if HAS_WV:
                ...
                def load_webview(dt):
                    if PORT:
                        self.web.load_url(f'http://localhost:{PORT}')
                        self.status.text = 'Bot running'
                    else:
                        Clock.schedule_once(load_webview, 0.5)
                Clock.schedule_once(load_webview, 1)
            else:
                self.label = Label(text='Waiting for server...', halign='center')
                layout.add_widget(self.label)
                def update_label(dt):
                    if PORT:
                        self.label.text = f'Open http://localhost:{PORT} in browser'
                    else:
        ...
        def on_stop(self):
            ...
            if ANDROID and wakeLock is not None:
                try:
                    ...
                    pass
            android_notify("Trading Bot", "Bot stopped")
            save_all_state()
            save_config()
            save_watchlist()
