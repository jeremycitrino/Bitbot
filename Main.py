#!/usr/bin/env python
# nostop_android.py – Full trading bot for Android APK
# Uses yfinance, caching, REST polling, Web dashboard, and Android native features.
...
# ===============================
#  ANDROID NATIVE IMPORTS (optional)
# ===============================
...
if ANDROID:
    try:
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, 'TradingBot::WakeLock')
        wakeLock.acquire()
        print("✅ Android WakeLock acquired")
    except Exception as e:
        print(f"⚠️ WakeLock error: {e}")

    try:
        from plyer import notification
        def notify_foreground(title, msg):
            notification.notify(title=title, message=msg, app_name='TradingBot', timeout=0)
        notify_foreground("Trading Bot", "Bot is active")
    except Exception as e:
...
# ===============================
#  ORIGINAL BOT CONFIGURATION (unchanged)
# ===============================
...
# ===============================
#  PERSISTENCE & UTILITIES (unchanged)
# ===============================
def save_watchlist():
    global WATCHLIST
    try:
...
def prevent_sleep():
    # Keep the CPU active via Android WakeLock (already acquired) and background threads
    threading.Thread(target=auto_save_loop, daemon=True).start()
    threading.Thread(target=heartbeat_loop, daemon=True).start()
    # Periodic foreground notification reminder
    def notify_loop():
        while running:
            time.sleep(300)  # every 5 minutes
            if running:
                notify_foreground("Trading Bot", f"Active | P:{len(positions)} | Net: ${cash+sum(...):.0f}")
    threading.Thread(target=notify_loop, daemon=True).start()
    logger.info("Background services: ACTIVE")

# ===============================
#  PRICE & INDICATORS (unchanged)
# ===============================
...
# ===============================
#  BOT START FUNCTION (for background thread)
# ===============================
def start_bot_backend():
    ...
    load_config()
    load_all_state()
    prevent_sleep()  # starts background threads
    ...
    for offset in range(12):
        ...
        try:
            ...
            continue
    if PORT is None:
        logger.error("Could not find an available port in range 8888-8899.")
        return

    server = HTTPServer(('0.0.0.0', PORT), Handler)
    threading.Thread(target=server.serve_forever, daemon=True).start()

    # keepalive thread for the web server
    def keepalive():
        time.sleep(30)
        while running:
            try:
    ...
    logger.info("="*50)
    logger.info("   REST BOT - ANDROID APK VERSION")
    logger.info("="*50)
...
# Try to import Android WebView (if available)
try:
...
class TradingBotApp(App):
    def build(self):
        ...
        threading.Thread(target=start_bot_backend, daemon=True).start()
        # Status bar
        self.status_btn = Button(text='Starting bot...', size_hint=(1, 0.08), background_normal='', background_color=(0.2,0.3,0.4,1))
        self.layout.add_widget(self.status_btn)
        # WebView or fallback
        if ANDROID_WEBVIEW:
            self.webview = WebView()
            self.layout.add_widget(self.webview)
            # Wait a few seconds for the server to start, then load URL
            Clock.schedule_once(lambda dt: self.load_webview(), 3)
        else:
            self.label = Label(text='Web dashboard available at http://localhost:8888\nOpen in browser.', halign='center', valign='middle')
    ...
    def load_webview(self):
        if hasattr(self, 'webview') and self.webview:
            self.webview.load_url('http://localhost:{}'.format(PORT if PORT else 8888))
            self.status_btn.text = 'Bot running'
            self.status_btn.background_color = (0.1,0.6,0.1,1)

    def on_stop(self):
        global running
        running = False
        if ANDROID and wakeLock is not None:
            try:
                wakeLock.release()
                print("WakeLock released")
            except:
                pass
        notify_foreground("Trading Bot", "Bot stopped")
        save_all_state()
        save_config()
        save_watchlist()
